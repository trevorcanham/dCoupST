function [measured_central_moments,imposed_features,x,lut] = impose_n_measure_features_reverseLut(x,idLut,desired_features, rel_tol)
...
[t,lutx] = ode45(@(t,x) grad_n(t,x,n,negative_grad),[0:NN],idLut, opt);
%size(x)
x0 = x(end,:);
lut0 = lutx(end,:);
...
lut = (lut0-media)*sqrt(D(2)/variance) + D(1);
media = mean(x0);
variance = mean((x0-media).^2);
